...
class TestLiveOwnerGrace(unittest.TestCase):
    """A journal whose owner process is STILL ALIVE must not be reclaimed.

    A live owner may be mid-op-loop; rolling its batch back underneath it would
    corrupt its own result. Reclaiming it is only safe after
    ``_LIVE_OWNER_GRACE_S`` — comfortably past the tool executor's 420 s
    abandon deadline — so a slow-but-live batch is never reclaimed early.
    """

    def setUp(self):
        self.home = tempfile.mkdtemp(prefix="skmbatch_live_")
        os.environ["HERMES_HOME"] = self.home
        os.environ["HERMES_YOLO_MODE"] = "1"
        os.makedirs(os.path.join(self.home, "skills"), exist_ok=True)
        import importlib
    ...
    def test_live_owner_with_a_fresh_transaction_is_left_alone(self):
        import subprocess
        ...
        # A real, live foreign process — not this one, so the pid check applies.
        owner = subprocess.Popen([sys.executable, "-c", "import time; time.sleep(60)"])
        self.addCleanup(owner.kill)

        txn = smb._txn_root() / "live-owner-batch"
        txn.mkdir(parents=True)
        (txn / "journal.json").write_text(json.dumps({
            "batch_id": txn.name, "pid": owner.pid, "started_at": _time.time(),
            "skills": [{"name": "probe", "pre_dir": None, "snap": None}],
        }), encoding="utf-8")

        note = smb._recover_one(txn, smt._find_skill)

        self.assertIsNone(note, f"a live owner's batch was reclaimed: {note}")
        self.assertTrue(txn.is_dir(), "a live owner's transaction dir must survive")
