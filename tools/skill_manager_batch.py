...
# Durable transaction root: a SIBLING of <hermes_home>/skills, never inside it. Snapshots hold
# SKILL.md copies, and any directory under a skills root that is not in EXCLUDED_SKILL_DIRS would
# surface as a phantom skill to every scanner (_find_skill, skill_view, the curator).
_BATCH_TXN_DIRNAME = ".skill-batch-txn"
_JOURNAL_NAME = "journal.json"
# A journal whose owner process is STILL ALIVE is reclaimed only after this grace: a live owner
# may still be inside its op loop, and rolling it back underneath itself would corrupt its result.
# Comfortably above the tool executor's own abandon deadline (420 s, agent/tool_executor.py) so a
# slow-but-live batch is never reclaimed early.
_LIVE_OWNER_GRACE_S = 900.0
# Transaction dirs in flight in THIS process, by dir name: a live batch is never "interrupted".
